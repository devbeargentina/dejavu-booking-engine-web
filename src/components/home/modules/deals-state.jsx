...
import { FetchPackageByState } from "../../../redux/packageSliceNew";
import flight from "../../../assets/images/web/states/gujarat.jpg";

const getImageSrc = async (name) => {
  const image = await import(`../../../assets/images/web/states/${name==="asam" ? "gujarat" : name}.jpg`);
  return image.default;
};
const DealsStateWise = (props) => {
  ...
  const { loading, packagesByState } = useSelector((state) => state.package);

  useEffect(() => {
    if (!packagesByState || packagesByState.length === 0) {
      dispatch(FetchPackageByState());
    }
  }, [dispatch]);
...
`;
  ...
  const mappedArray = Object.entries(packagesByState).map(([key, value]) => {
